...
function getSubmenu2(cat) {
    ...
    $.ajax({
        ...
        success: function (data) {
            var markup = "";
            for (var x = 0; x < data.length; x++) {
                markup += "<a href='http://shop.janere.ee/Home/Products?groupno=" + data[x].Value + "'><div id=cate" + data[x].Value + " class='submenu2' style='color:#666666;margin: 2px 0px;padding: 9px;font-size:18px;font-weight: bold;'>" + data[x].Text + "</div></a>";
            }
...
function GetInterFace() {
    ...
    $.ajax({
        ...
        error: function (reponse) {
            alert("error GetInterFace: " + reponse);
        }
    });
}
