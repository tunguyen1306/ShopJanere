...
namespace WebApplication1.Controllers
{
    public class ProductController : Controller
    {
        ...
        public ActionResult IndexAjax(string tab="all",int start = 0, int view = 10, int cat = 0, int str = 0, int stk = 0, int wh = 0)
        {
            ...
            if (str != 0)
            {
                listProduct = listProduct.Where(x => x.CATEGORYNO == str);
            }
            if (stk != 0)
            {
                listProduct = listProduct.Where(x => x.CATEGORYNO == stk);
            }
            if (wh != 0)
            {
                listProduct = listProduct.Where(x => x.CATEGORYNO == wh);
            }
            ...
            var listAll = (from pro in listProduct
                join cate in db.categories on pro.CATEGORYNO equals cate.CATEGORYNO
                 select new {tblitem = pro ,tblCategory = cate } );
           ...
           /* ViewBag.CategoryList = db.categories.ToList();
            ...
            ViewBag.stockList = db.stockcods.ToList();
            ViewBag.StoreList = db.stores.ToList();*/
            var datas = db_data.Select(t => new AllModel { tblCategory = t.tblCategory, tblitem = t.tblitem }).ToList();
